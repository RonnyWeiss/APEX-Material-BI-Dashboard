var apexBIDashBoard = function (apex, $) {
    "use strict";
    var util = {
        "featureDetails": {
            name: "Material-BI-Dashboard",
            scriptVersion: "1.0.0.23",
            utilVersion: "1.6",
    ...
    return {
        initialize: function (pRegionID, pRegionIDRefresh, pAjaxID, pNoDataMsg, pErrMsg, pDefaultConfigJSON, pLoadingMethod, pFeatures, pChartConfigJSON, pItems2Submit, pItems2SubmitImg, pRequireHTMLEscape, pPurifyOptions, pMapURL, pMapOptions, pTableConfigJSON, pClockOptions, pCalendarOptions) {
            ...
            var containerIDSel = "#" + pRegionID;
            var container = $(containerIDSel);
            var loaderSel = "#" + pRegionIDRefresh;
            var masonry;
            var masonrySizeCheckIntveral;
            var mapJSON = {};
            var timers = {};
            var keepScroll = 0;
            timers.defTime = 500;
            var resizeRange = 5;
            var resizeRangeLarge = 10;
            var sizeCorrection = 10;
            var asyncAjaxReference = 0;
            ...
            if (pRegionID && container.length > 0) {
                ...
                /* default d3 map options */
                var stdMapConfigJSON = {
                    "tour": {
                        "enabled": true,
                        "direction": "right",
                        "speed": 2,
                ...
                if (util.isDefinedAndNotNull(pFeatures)) {
                    var featureArr = pFeatures.split(':');
                }
                ...
                /* merge config for d3 map if feature is activated */
                ...
                if (configJSON.refresh > 0) {
                    var bidaRefresh;
                    bidaRefresh = setInterval(function () {
            ...
            function getData(pLoadingMethod, pDefaultConfig, pIsRefresh) {
                ...
                var submitItems = pDefaultConfig.items2Submit;
            ...
            function drawGridItems(pAjaxResponse, pDefaultConfig) {
                ...
                var masonryIdentifier = "." + pRegionID;
                ...
                var sizer = $("<div></div>");
                sizer.addClass("bida-container-sizer");
                sizer.addClass(pRegionID + "-sizer");
                ...
                if (pAjaxResponse.items && pAjaxResponse.items.length > 0) {
                    ...
                    $.each(pAjaxResponse.items, function (idx, ajaxItems) {
                        var itemWidth = pDefaultConfig.colSpan;
            ...
            function drawItemRegion(pitemIdx, pItemWidth, pParent, pAjaxItems, pDefaultConfig) {
                ...
                var thisAjaxItems = util.jsonSaveExtend(null, pAjaxItems);
                var itemRegionID = pRegionID + "--" + pitemIdx;
                var itemRegionIDSel = "#" + itemRegionID;
                /* calc item height (currently min height) */
                var itemHeight = (pAjaxItems.height || pDefaultConfig.height) - sizeCorrection;
                ...
                var isAsync = setObjectParameter(pAjaxItems.isAsync, false, true);
                ...
                if (pLoadingMethod === "A" && isAsync) {
                    ...
                    if (pAjaxItems.refresh > 0) {
                        var itemTimer = setInterval(function () {
            ...
            function prepareItemContent(pitemIdx, pItemHeight, pItemSel, pAjaxItems, pDefaultConfig, pIsRefresh) {
                ...
                var div = $(pItemSel);
                var itemHeight = pItemHeight;
                var loaderWaitTime = 100;

                /* update layoput and keep scroll position */
                function changeMasonryLayout() {
                    if (masonry) {
                        masonry.layout();
                ...
                /* workaround if masonry could not calculate item correct e.g. in APEX Tabs */
                function checkMasonrySize() {
                    if (!masonrySizeCheckIntveral) {
                        function startResizeWA() {
                            masonrySizeCheckIntveral = setInterval(function () {
                                if ($(container).length === 0) {
                                    clearInterval(masonrySizeCheckIntveral);
                                } else {
                                    if ($(container).is(":visible")) {
                                        var scrollHeight = document.getElementById(pRegionID).scrollHeight;
                                        var innerHeight = $(container).height();
                                        if (!util.isBetween(innerHeight, scrollHeight, resizeRangeLarge)) {
                                            apex.debug.info({
                                                "moudule": "checkMasonrySize",
                                                "msg": "Masonry has resize problem",
                                                "featureDetails": util.featureDetails,
                ...
                if (pIsRefresh || (pLoadingMethod === "A" && isAsync)) {
                    if (util.isDefinedAndNotNull(pAjaxItems.itemID)) {
                        ...
                        function onAsyncSuccess(pAsyncAjaxItems) {
                            /* merge sync json data with async json data if something is missing */
                            var newAsyncAjaxItems = util.jsonSaveExtend(pAjaxItems, pAsyncAjaxItems);
                        ...
                        apex.server.plugin(
                            ...
                            success: function (pData, pMethod, pRequest) {
                                ...
                                if (pRequest.getResponseHeader("ajax-reference") == asyncAjaxReference) {
            ...
            function drawSubText(pPosition, pParent, pText, pDefaultConfig, pIsSafeItem) {
                ...
                var subtextClass = "bida-item-subtext-" + pPosition;
                var subtext = $("<div></div>");

                var subtextExists = $(pParent).find("." + subtextClass);
                ...
                var subtextContent = escapeOrSanitizeHTML(pText, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
            ...
            function drawItemTitle(pParent, pText, pBackColor, pColor, pIcon, pIconColor, pDefaultConfig, pIsSafeItem) {
                ...
                var titleClass = "bida-item-title";
                var title = $("<div></div>");

                var titleExists = $(pParent).find("." + titleClass);
                ...
                var titleContent = escapeOrSanitizeHTML(pText, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                titleText.html(titleContent);
                titleText.attr("title", titleText.text());
                title.html(titleText);

                /* add title icon */
                var icon = pIcon || pDefaultConfig.title.icon;
                var iconColor = pIconColor || pDefaultConfig.title.iconColor;
             ...
             **
             ***********************************************************************/
            function getRestHeight(pItemSel, pItemHeight, exclude) {
                var ret = pItemHeight;
            ...
            function drawItemRegionContent(pitemIdx, pItemSel, pItemHeight, pAjaxItems, pDefaultConfig) {
                ...
                if (util.isDefinedAndNotNull(pAjaxItems.optionsLink)) {
                    ...
                    var faSrcString = "fa-";
                    var optionsLinkIcon = $("<i></i>");

                    var optionsIcon = pAjaxItems.optionsLinkIcon || pDefaultConfig.optionsLink.icon;
                ...
                if (util.isDefinedAndNotNull(pAjaxItems.optionsLinkTop)) {
                    ...
                    /* check if it should be an icon or a background image */
                    var faSrcString = "fa-";
                    var optionsLinkTopIcon = $("<i></i>");

                    var optionsIcon = pAjaxItems.optionsLinkTopIcon || pDefaultConfig.optionsLinkTop.icon;
                    if (util.isDefinedAndNotNull(optionsIcon) && optionsIcon.substr(0, faSrcString.length) === faSrcString) {
                ...
                var marked = setObjectParameter(pAjaxItems.isMarked, false, true);
                ...
                if (util.isDefinedAndNotNull(pAjaxItems.itemType)) {
                    ...
                    if (util.isDefinedAndNotNull(pAjaxItems.itemData) || pAjaxItems.itemType === "calendar" || pAjaxItems.itemType === "note" || pAjaxItems.itemType === "clock") {
                        ...
                        switch (pAjaxItems.itemType) {
                            ...
                            default:
                                ...
                                apex.debug.error({
                                    "fct": util.featureDetails.name + " - " + "drawItemRegionContent",
                                    "msg": "itemType not found (html, card, list, map, table, clock, calendar)",
                                    "featureDetails": util.featureDetails
                                });
                        }
                    } else {
                    ...
                    if (util.isDefinedAndNotNull(pAjaxItems.subText2)) {
                        var subText2 = $(pItemSel).find(".bida-item-subtext-2");
            ...
            function drawHTML(pItemSel, pItemHeight, pItemData, pItemID, pDefaultConfig, pOversize, pIsSafeItem) {
                ...
                function renderHTML(pContent) {
                    var content = escapeOrSanitizeHTML(pContent, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                    ...
                    if ($.inArray("inlineimagesdownload", pDefaultConfig.features) > -1) {
                        var imgItems = htmlDiv.find('img[alt*="aih#"]');
                        $.each(imgItems, function (idx, imgItem) {
                            ...
                            if (pk) {
                                ...
                                var submitItems = pDefaultConfig.items2SubmitImg;
                                var imgSRC = apex.server.pluginUrl(pAjaxID, {
                                    x01: "imageDownload",
                                    x02: pItemID,
                                    x03: pk,
                                    pageItems: submitItems
                ...
                if (util.isDefinedAndNotNull(pItemData.header) && pItemData.header.length > 0) {
                    $.each(pItemData.header, function (idx, headerData) {
                        if (util.isDefinedAndNotNull(headerData.text)) {
                            ...
                            var content = escapeOrSanitizeHTML(headerData.text, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                            header.html(content);
                    ...
                    $.each(pItemData.data, function (idx, rowData) {
                        ...
                        $.each(rowData, function (cIdx, cellData) {
                            if (util.isDefinedAndNotNull(cellData.text)) {
                                ...
                                var content = escapeOrSanitizeHTML(cellData.text, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                                cell.html(content);
                                row.append(cell);
                            }
                        });
            ...
            function drawList(pItemSel, pItemHeight, pItemData, pDefaultConfig, pOversize, pIsSafeItem) {
                    ...
                    $.each(pGroupData, function (i, listItem) {
                        ...
                        if (util.isDefinedAndNotNull(listItem.icon) || util.isDefinedAndNotNull(listItem.iconText)) {
                            ...
                            if (listItem.icon && listItem.icon.substr(0, faSrcString.length) === faSrcString) {
                                ...
                                var listItemStdStyle = 'linear-gradient(60deg, hsl(' + (i * 23) % 350 + ', 79%, 45%), hsl(' + (i * 23) % 350 + ', 60%, 45%))';

                                icon.css("background", listItem.iconBackColor || listItemStdStyle);
                            } else {
                                if (util.isDefinedAndNotNull(listItem.iconText)) {
                                    var iconContent = escapeOrSanitizeHTML(listItem.iconText, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                                    var iconSpan = $("<span></span>");
                        ...
                        if (util.isDefinedAndNotNull(listItem.title)) {
                            var listTitle = $("<span></span>");
                            listTitle.addClass("list-item-title");
                            listTitle.addClass("truncate-text");

                            var listTitleContent = escapeOrSanitizeHTML(listItem.title, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                        ...
                        if (util.isDefinedAndNotNull(listItem.text)) {
                            var listText = $("<span></span>");
                            listText.addClass("list-item-text");

                            var listTextContent = escapeOrSanitizeHTML(listItem.text, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                ...
                var groupsArr = util.groupObjectArray(pItemData, "group");
                ...
                $.each(groupsArr, function (groupName, groupData) {
                    ...
                    var groupTitle = escapeOrSanitizeHTML(groupName, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
            ...
            function drawBadges(pIndex, pItemHeight, pItemSel, pItemData, pDefaultConfig, pIsSafeItem) {
                ...
                var faSrcString = "fa-";
                var isImage = false;
                var lnHeightFactor = 0.98;
                var factorTitle = 0.15;
                var factorIcon = 0.28;
                var factorValue = 0.28;
                var factorTitleValueDiff = factorValue - factorTitle;
                var cnt = 0;
                var stdFontColor = 'linear-gradient(60deg, hsl(' + (pIndex * 23) % 350 + ', 79%, 45%), hsl(' + (pIndex * 23) % 350 + ', 60%, 45%))';
                ...
                if (util.isDefinedAndNotNull(pItemData.title)) {
                    ...
                    var title = escapeOrSanitizeHTML(pItemData.title, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                ...
                if (util.isDefinedAndNotNull(pItemData.icon)) {
                    ...
                    if (util.isDefinedAndNotNull(pItemData.icon) && pItemData.icon.substr(0, faSrcString.length) === faSrcString) {
                        ...
                        if (util.isDefinedAndNotNull(pItemData.iconText)) {
                            var iconContent = escapeOrSanitizeHTML(pItemData.iconText, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                ...
                function resize() {
                    var height = pItemHeight;
                    var size = Math.min(height, badgeDiv.width());
                    if (spanTitle) {
                ...
                /* dirty workaround dynamic font size is impossible with css and svg solution is not working on ie and safari */
                var oldWidth = badgeDiv.width();
                ...
                function startResizeWA() {
                    timers.innerItemsIntervals[pItemSel] = setInterval(function () {
                        if ($(pItemSel).length === 0) {
                            ...
                            if (badgeDiv.is(":visible")) {
                                if (!util.isBetween(badgeDiv.width(), oldWidth, resizeRange)) {
                                    oldWidth = badgeDiv.width();
                                    resize();
                                }
                            }
                        }
            ...
            function drawCard(pIndex, pItemHeight, pItemSel, pItemData, pDefaultConfig, pOversize, pIsSafeItem) {
                ...
                if (util.isDefinedAndNotNull(pItemData.icon) || util.isDefinedAndNotNull(pItemData.iconText)) {
                    ...
                    if (util.isDefinedAndNotNull(pItemData.icon) && pItemData.icon.substr(0, faSrcString.length) === faSrcString) {
                        ...
                        var cardStdStyle = 'linear-gradient(60deg, hsl(' + (pIndex * 23) % 350 + ', 79%, 45%), hsl(' + (pIndex * 23) % 350 + ', 60%, 45%))';
                ...
                if (util.isDefinedAndNotNull(pItemData.value)) {
                    ...
                    var cardTitleContent = escapeOrSanitizeHTML(pItemData.value, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                ...
                if (util.isDefinedAndNotNull(pItemData.valueSmall)) {
                    ...
                    var cardSubTitle = $("<p></p>");
                    cardSubTitle.addClass("card-sub-title");

                    var cardSubTitleContent = escapeOrSanitizeHTML(pItemData.valueSmall, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                ...
                if (util.isDefinedAndNotNull(pItemData.footer)) {
                    ...
                    var cardFooterStatsContent = escapeOrSanitizeHTML(pItemData.footer, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
            ...
            /***********************************************************************
             **
             ** function to render d3 analog clock
             **
             ***********************************************************************/
            function drawClock(pItemSel, pItemHeight, pItemData, pItemConfig, pDefaultConfig) {
                ...
                try {
                    var pCon = $(pItemSel);
                    var clockID = $(pItemSel).attr("id") + "-clock";
                    var shadowID = $(pItemSel).attr("id") + "-shadow";
                    var clockIDSel = "#" + clockID;
                    var clockHandsID = $(pItemSel).attr("id") + "-clockhands";
                    var clockHandsIDSel = "#" + clockHandsID;
                    ...
                    var hourTickLength;
                    var hourLabelRadius;
                    var hourLabelYOffset;
                    var hourLabelSize;
                    var secondLabelSize;
                    var clockLabelYOffset;
                    var clockLabel2YOffset;
                    var clockLabelFontSize;
                    var clockHandCoverSize;
                    var secondLabelRadius;
                    var secondLabelYOffset;
                    var svg;
                    var clockElements;
                    var hands;
                    var hourScale;
                    ...
                    var factor;
                    var handData;
                    var clockCircleRadius;
                    var diff = 0;
                    var time = new Date();
                    var secondLabel = setObjectParameter(pItemConfig.secondLabel, pDefaultConfig.clockOptions.second.label, true);
                    var shadows = setObjectParameter(pItemConfig.shadows, pDefaultConfig.clockOptions.shadows, true);
                    var circleWidth = setObjectParameter(pItemConfig.circleWidth, pDefaultConfig.clockOptions.circle.width);
                    var filterUrl;
                    ...
                    if (util.isDefinedAndNotNull(pItemData) && util.isDefinedAndNotNull(pItemData.startDate)) {
                        try {
                            var givenTime = Date.parse(pItemData.startDate);
                            diff = givenTime - time;
                        } catch (e) {
                            apex.debug.error({
                                "fct": util.featureDetails.name + " - " + "drawClock",
                    ...
                    function formatLabelStr(pLabel) {
                        var str = "";
                        if (util.isDefinedAndNotNull(pLabel)) {
                            var lang = apex.locale.getLanguage();
                            str = pLabel.replace('#date#', time.toLocaleDateString(lang));
                    ...
                    function renderClock() {
                        ...
                        if (!svg) {
                            svg = d3.select(pItemSel).append("svg")
                                .attr("id", clockID)
                        ...
                        if (util.isDefinedAndNotNull(pItemData) && util.isDefinedAndNotNull(pItemData.label)) {
                            var label = formatLabelStr(pItemData.label);
                        ...
                        if (util.isDefinedAndNotNull(pItemData) && util.isDefinedAndNotNull(pItemData.label2)) {
                            var label2 = formatLabelStr(pItemData.label2);
                    ...
                    /* execute resize */
                    function resize() {
                        if (!document.hidden && pCon.is(":visible")) {
                            renderClock();
                        }
                    }
                    ...
                    function startResizeWA() {
                        timers.innerItemsIntervals[pItemSel] = setInterval(function () {
                            if ($(pItemSel).length === 0) {
                                ...
                                if (!util.isBetween(pCon.width(), pCon.find("svg").width(), resizeRange)) {
                                    apex.debug.info({
                                        "fct": util.featureDetails.name + " - " + "drawClock",
                                        "msg": "Clock has resize problem",
                                        "featureDetails": util.featureDetails
                                    });
                                    resize();
                                }
            ...
            function drawCalendar(pItemSel, pItemHeight, pItemData, pItemConfig, pDefaultConfig, pOversize, pIsSafeItem) {
                ...
                var pCon = $(pItemSel);
                var calendarID = pCon.attr("id") + "-calendar";
                var calendarIDSel = "#" + calendarID;
                var locale = apex.locale.getLanguage();

                var calendarCon = $("<div></div>");
                calendarCon.addClass("bida-calendar");
                calendarCon.attr("id", calendarID);

                pCon.append(calendarCon);

                var eventLimit = setObjectParameter(pItemConfig.eventLimitPerDay, pDefaultConfig.calendarOptions.eventLimitPerDay);
                var displayTime = setObjectParameter(pItemConfig.displayTime, pDefaultConfig.calendarOptions.displayTime, true);
                var viewType = setObjectParameter(pItemConfig.viewType, pDefaultConfig.calendarOptions.viewType);
                var hours12 = setObjectParameter(pItemConfig.hours12, pDefaultConfig.calendarOptions.hours12, true);
                var timeGridStartTime = setObjectParameter(pItemConfig.timeGridStartTime, pDefaultConfig.calendarOptions.timeGridStartTime);
                ...
                var calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    eventMouseEnter: function (info) {
                        ...
                        var htmlStr = escapeOrSanitizeHTML(info.event.title, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                        ...
                        if (util.isDefinedAndNotNull(info.event.extendedProps.details)) {
                            var str = escapeOrSanitizeHTML(info.event.extendedProps.details, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
            ...
            function drawNoteEditor(pItemID, pItemSel, pItemHeight, pItemData, pItemConfig, pDefaultConfig, pOversize, pIsSafeItem) {
                ...
                var pCon = $(pItemSel);
                var noteID = pCon.attr("id") + "-note";
                var noteIDSel = "#" + noteID;
                    ...
                    actions: [
                        {
                            ...
                            title: "Save",
                            result: function () {
                                var content = noteCon.find(".pell-content");
                                util.loader.start(pItemSel);
                                disableSaveButton();

                                var clob = content.html();
                ...
                var content = noteCon.find(".pell-content");
                var actionBarHeight = noteCon.find(".pell-actionbar").height();
            ...
            function drawD3Map(pItemSel, pItemHeight, pItemData, pItemConfig, pDefaultConfig, pIsSafeItem) {
                ...
                try {
                    var mapCont = $(pItemSel);
                    var isTour = setObjectParameter(pItemConfig.tourEnabled, pDefaultConfig.mapOptions.tour.enabled, true);
                    var tourSpeed = setObjectParameter(pItemConfig.tourSpeed, pDefaultConfig.mapOptions.tour.speed) / 2000;
                    var tourStart = setObjectParameter(pItemConfig.tourStart, pDefaultConfig.mapOptions.tour.start);
                    var circleShadows = setObjectParameter(pItemConfig.circlesShadows, pDefaultConfig.mapOptions.circles.shadows, true);
                    var circlesRadius = setObjectParameter(pItemConfig.circlesRadius, pDefaultConfig.mapOptions.circles.radius);
                    var circlesColor = setObjectParameter(pItemConfig.circlesColor, pDefaultConfig.mapOptions.circles.color);
                    var circlesOpacity = setObjectParameter(pItemConfig.circlesOpacity, pDefaultConfig.mapOptions.circles.opacity);
                    var circlesOpacityHover = setObjectParameter(pItemConfig.circlesOpacityHover, pDefaultConfig.mapOptions.circles.opacityHover);
                    var tourShadowEffect = setObjectParameter(pItemConfig.tourShadowEffect, pDefaultConfig.mapOptions.tour.shadowEffect, true);
                    var centerLongi = setObjectParameter(pItemConfig.mapCenterLongitude, pDefaultConfig.mapOptions.map.centerLongitude);
                    var centerLati = setObjectParameter(pItemConfig.mapCenterLatitude, pDefaultConfig.mapOptions.map.centerLatitude);
                    var initialZoom = setObjectParameter(pItemConfig.mapInitialZoom, pDefaultConfig.mapOptions.map.initialZoom);
                    var zoomEnabled = setObjectParameter(pItemConfig.mapZoomEnabled, pDefaultConfig.mapOptions.map.zoomEnabled, true);
                    var mapColor = setObjectParameter(pItemConfig.mapColor, pDefaultConfig.mapOptions.map.color);
                    var mapStroke = setObjectParameter(pItemConfig.mapStroke, pDefaultConfig.mapOptions.map.stroke);
                    var mapStrokeWidth = setObjectParameter(pItemConfig.mapStrokeWidth, pDefaultConfig.mapOptions.map.strokeWidth);
                    var mapShadows = setObjectParameter(pItemConfig.mapShadows, pDefaultConfig.mapOptions.map.shadows, true);
                    var shadowID = $(pItemSel).attr("id") + "-shadow";
                    var shadowURL;
                    var mapShadowURL;
                    var circlehadowURL;
                    ...
                    if (tourShadowEffect) {
                        var oceanID = $(pItemSel).attr("id") + "-ocean";
                        var highlightID = $(pItemSel).attr("id") + "-highlight";
                        var shadingID = $(pItemSel).attr("id") + "-shading";
                        var oceanURL = "url(#" + oceanID + ")";
                        var highlightURL = "url(#" + highlightID + ")";
                        var shadingURL = "url(#" + shadingID + ")";
                        var ocean;
                        var highlight;
                        var shading;
                    }
                    ...
                    /* function to render d3 map when map json is loaded */
                    function render(data) {
                        ...
                        if (tourShadowEffect) {
                            var oceanEff = svg.append("defs").append("radialGradient")
                                .attr("id", oceanID)
                                .attr("cx", "75%")
                                .attr("cy", "25%");
                            ...
                            var highlightEff = svg.append("defs").append("radialGradient")
                                .attr("id", highlightID)
                                .attr("cx", "75%")
                                .attr("cy", "25%");
                            ...
                            var shadingEff = svg.append("defs").append("radialGradient")
                                .attr("id", shadingID)
                                .attr("cx", "55%")
                                .attr("cy", "45%");
                        ...
                        circles = g.selectAll(".bida-map-circle")
                            ...
                            .on("mouseover", function (event, d) {
                                ...
                                if (util.isDefinedAndNotNull(d.tooltip)) {
                                    var ttStr = escapeOrSanitizeHTML(d.tooltip, pDefaultConfig, pIsSafeItem, pRequireHTMLEscape);
                        ...
                        function projectionChanged() {
                            ...
                            circles
                                ...
                                .attr("r", function (d) {
                                    if (isTour) {
                                        var gdistance = d3.geoDistance([d.longitude, d.latitude], projection.invert(getCenter()));
                                        if (gdistance > 1.57) {
                                            return 0;
                                        }
                                    }
                        ...
                        /* execute resize */
                        function resize() {
                            if (!document.hidden && mapCont.is(":visible")) {
                        ...
                        function startResizeWA() {
                            timers.innerItemsIntervals[pItemSel] = setInterval(function () {
                                if ($(pItemSel).length === 0) {
                                    ...
                                    if (!util.isBetween(mapCont.width(), mapCont.find("svg").width(), resizeRange)) {
                                        apex.debug.info({
                                            "fct": util.featureDetails.name + " - " + "drawD3Map",
                                            "msg": "Map has resize problem",
                                            "featureDetails": util.featureDetails
                                        });
                                        resize();
                                    }
                    ...
                    if (isTour) {
                        if (!mapJSON.tour) {
                            var url = pMapURL + "world-tour.json";
                        ...
                        if (!mapJSON.zoom) {
                            var url = pMapURL + "world-zoom.json";
                            $.getJSON(url, function (data) {
                                mapJSON.zoom = data;
                                render(mapJSON.zoom);
                            });
